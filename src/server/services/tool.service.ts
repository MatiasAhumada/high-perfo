...
export const toolService = {
  async findByRoutine(routineId: string, accountId: string, user: SessionUser) {
    ...
    const routine = await assignedRoutineRepository.findByIdWithTools(routineId)
    if (!routine) {
      throw new ApiError({
  ...
  async create(routineId: string, dto: CreateToolInput, accountId: string, user: SessionUser) {
    requireAccountAccess(user, accountId)
    const routine = await assignedRoutineRepository.findByIdWithTools(routineId)
    if (!routine) {
      throw new ApiError({
  ...
  async update(
    ...
    const routine = await assignedRoutineRepository.findByIdWithTools(tool.assignedRoutineId)
    if (!routine) {
      throw new ApiError({
        status: httpStatus.NOT_FOUND,
        message: ERROR_MESSAGES.ROUTINE_NOT_FOUND,
