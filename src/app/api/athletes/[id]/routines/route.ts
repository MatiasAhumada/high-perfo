...
export async function GET(
  ...
  try {
    ...
    const statusParam = request.nextUrl.searchParams.get("status") as import("@prisma/client").RoutineStatus | null
    const options = statusParam ? { status: statusParam } : {}
    const routines = await routineService.findByAthlete(athleteId, accountId, user, options)
...
export async function POST(
  ...
  try {
    ...
    const routine = await routineService.assign(athleteId, assignDto, user.id, accountId, user)
