...
export function RoutineHeader({ onSendEmail }: RoutineHeaderProps) {
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-headline-md font-display text-on-surface tracking-tight">
          {UI_TEXTS.ROUTINES.TITLE}
        </h2>
        <p className="text-sm text-on-surface-variant/50 mt-1 font-body">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <div className="flex bg-surface-container-low rounded-lg border border-outline-variant/30 p-0.5">
          {segments.map((seg) => (
            <button
              key={seg.key}
              className="px-3 py-1.5 text-label-caps uppercase tracking-wider text-on-surface-variant/50 hover:text-on-surface-variant transition-colors rounded-md text-xs font-display first:bg-on-tertiary-container/10 first:text-on-tertiary-container first:font-semibold"
            >
              {seg.label}
            </button>
          ))}
