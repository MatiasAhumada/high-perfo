  ...
  const loadData = async () => {
    try {
      ...
      setStats({
        ...
        totalAtletas: accountsData.reduce(
          (sum, acc) => sum + (acc._count?.athletes || 0),
          0,
        ),
      });
    } catch (error) {
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 w-full">
        {[0, 1, 2].map((i) => (
          <div
            key={i}
            className="h-36 rounded-xl bg-surface-container border border-outline-variant/30 animate-pulse"
          />
        ))}
      </div>
    );
          ...
          <table className="w-full min-w-[600px]">
            <thead>
              <tr className="border-b border-outline-variant/30">
                <th className="text-left text-xs text-on-surface-variant py-3 px-4 font-medium">
                  Nombre
                </th>
                <th className="text-left text-xs text-on-surface-variant py-3 px-4 font-medium hidden sm:table-cell">
                  Tipo
                </th>
                <th className="text-left text-xs text-on-surface-variant py-3 px-4 font-medium">
                  Usuarios
                </th>
                <th className="text-left text-xs text-on-surface-variant py-3 px-4 font-medium hidden md:table-cell">
                  Límite Coaches
                </th>
                <th className="text-left text-xs text-on-surface-variant py-3 px-4 font-medium">
                  Límite Atletas
                </th>
                <th className="text-left text-xs text-on-surface-variant py-3 px-4 font-medium hidden lg:table-cell">
                  Creado
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-outline-variant/20">
              {accounts.map((account) => (
                ...
                >
                  <td className="py-3 px-4 font-display text-sm text-on-surface">
                    {account.name}
                  </td>
