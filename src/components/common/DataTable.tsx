  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
        <div>
          <h3 className="font-display font-semibold text-headline-md text-on-surface leading-tight">
            {title}
          </h3>
          {subtitle && (
      ...
      <div className="bg-surface-container border border-outline-variant/30 rounded-lg overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[800px] border-collapse">
            <thead>
              <tr className="border-b border-outline-variant/30 bg-surface-container-low/50">
                {columns.map((column) => (
                  <th
                    key={column.key}
                    className={`font-data-mono text-data-mono text-on-surface-variant py-4 px-6 font-normal ${ALIGN_MAP[column.align ?? "left"] ?? "text-left"} ${column.className ?? ""}`}
                  >
                    {column.label}
                  </th>
                ))}
            ...
            <tbody className="font-body-base text-body-base text-on-surface divide-y divide-outline-variant/20">
              {loading ? (
                <tr>
                ...
                data.map((item, index) => (
                  ...
                  >
                    {columns.map((column) => (
                      <td
                        key={column.key}
                        className={`py-4 px-6 text-sm font-medium text-on-surface ${ALIGN_MAP[column.align ?? "left"] ?? "text-left"}`}
                      >
                        {column.render
                          ? column.render(item)
